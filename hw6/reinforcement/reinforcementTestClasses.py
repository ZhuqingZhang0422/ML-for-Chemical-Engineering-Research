...
class ApproximateQLearningTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        for n in self.numsExperiencesForDisplay:
            testPass, stdOutString, fileOutString, weights = self.executeNExperiencesNoGrade(moduleDict, n)  # added
            failureOutputFileString += self.prettyValueSolutionString('weights_k_%d' % n, pp.pformat(weights))  # added
            failureOutputStdString += stdOutString
            failureOutputFileString += fileOutString
            if not testPass:
                self.addMessage(failureOutputStdString)
                ...
                return self.testFail(grades)
        self.removeFailureFileIfExists()
        self.writeFailureFile(failureOutputFileString)  # added
        return self.testPass(grades)

    def executeNExperiencesNoGrade(self, moduleDict, n):
        # dump my own solutions
        qValuesPretty, weights, actions, lastExperience = self.runAgent(moduleDict, n)
        fileOutString = ''
        for action in actions:
            qValuesKey = 'q_values_k_%d_action_%s' % (n, action)
            qValues = qValuesPretty[action]
            fileOutString += self.prettyValueSolutionString(qValuesKey, qValues)
        return True, fileOutString, fileOutString, weights
...
class QLearningTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        for n in self.numsExperiencesForDisplay:
            ...
            testPass, stdOutString, fileOutString, valuesPretty, policyPretty = self.executeNExperiencesNoGrade(grades, moduleDict, solutionDict, n, checkValuesAndPolicy)  # added
            failureOutputStdString += stdOutString
            failureOutputFileString += fileOutString
            if not testPass:
                self.addMessage(failureOutputStdString)
                ...
                return self.testFail(grades)
        self.removeFailureFileIfExists()
        failureOutputFileString += self.prettyValueSolutionString('values', valuesPretty)  # added
        failureOutputFileString += self.prettyValueSolutionString('policy', policyPretty)  # added
        self.writeFailureFile(failureOutputFileString)  # added
        return self.testPass(grades)

    def executeNExperiencesNoGrade(self, grades, moduleDict, solutionDict, n, checkValuesAndPolicy):
        valuesPretty, qValuesPretty, actions, policyPretty, lastExperience = self.runAgent(moduleDict, n)
        fileOutString = ""
        ord_actions = ["north", "east", "exit", "south", "west"]
        for action in ord_actions:
            qValuesKey = 'q_values_k_%d_action_%s' % (n, action)
            qValues = qValuesPretty[action]
            fileOutString += self.prettyValueSolutionString(qValuesKey, qValues)
        return True, fileOutString, fileOutString, valuesPretty, policyPretty
